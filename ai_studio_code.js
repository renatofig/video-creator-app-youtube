...
async function runFullProcess(theme, runParams, webContents) {
    ...
    try {
        // --- 1. GERAÇÃO DE ROTEIRO ÚNICO (LÓGICA DE 2 ETAPAS) ---
        let themeNames = {'futebol':'Notícias de Futebol','pescados':'Notícias de Pescados','ciencia':'Curiosidades Científicas','historinhas':'Historinhas Infantis','freeform':'Prompt Livre'}
        ...
        if (['ciencia', 'historinhas', 'freeform'].includes(theme)) {
            ...
            while(brainstormTries-- > 0 && !uniqueTopic) {
                logToRenderer(webContents, '   - Fase 1: Brainstorm de tópicos com a IA...');
                let topicPrompt = '';

                switch (theme) {
                    case 'ciencia': topicPrompt = `Aja como um produtor de conteúdo científico. Gere uma lista de 10 títulos de vídeos sobre fatos surpreendentes ou descobertas pouco conhecidas em qualquer área da ciência (biologia, física, geologia, etc.). ${avoidanceInstruction} Responda APENAS com a lista, um título por linha.`; break;
                    case 'historinhas': topicPrompt = `Aja como um autor de livros infantis. Gere uma lista de 10 títulos para novas e originais historinhas. Os títulos devem sugerir personagens e cenários únicos. ${avoidanceInstruction} Responda APENAS com a lista, um título por linha.`; break;
                    case 'freeform': topicPrompt = `Aja como um roteirista criativo. O tema central é "${config.freeformPrompt}". Gere uma lista de 10 títulos de vídeo que abordem este tema de ângulos diferentes e inesperados. ${avoidanceInstruction} Responda APENAS com a lista, um título por linha.`; break;
                }
            ...
            if (!uniqueTopic) throw new Error("Não foi possível gerar um tópico único após várias tentativas.");
            
            let callToAction = theme === 'ciencia' ? callToActionCiencia : callToActionGenerico;
    ...
    if (runParams.upload && videoFinalPath && storyData) {
        const uploadSuccess = await uploadToYouTube(videoFinalPath, storyData.titulo, storyData.descricao, config, webContents, [], thumbnailPath);
